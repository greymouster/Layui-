...
$(function(){
   ...
   //解锁屏幕
   function unlockSystem(url,username,password){
        //
        $.ajax({
        	url:url,
        	type:'post',
        	...
        	success:function(data){
        		if(data.status>0) {
        			layer.msg('解锁成功',{icon:6,offset: ['55%', '45%']});
        			checkLockStatus(0);
        		}else{
        			checkLockStatus(1);
        			layer.msg('解锁失败',{icon:5,offset: ['55%', '45%']});
        		}
        	}
        });
   ...
   $('#unlock').click(function(){
	   ...
	   var password = $("#lock_password").val();
        unlockSystem(url,username,password);
   });
